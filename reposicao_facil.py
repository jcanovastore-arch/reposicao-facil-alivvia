# reposicao_facil.py - V11.4 (Final Clean - Estabilidade de Estado e Widget Fix)
# - FIX: Remove o 'value' redundante no st.text_input, resolvendo o warning de estado.
# - FIX: Simplifica get_padrao_from_sheets para confiar no logica_compra.py V11.4.
...
VERSION = "v11.4 – Estabilidade Total e Patch Definitivo"
...
# ===================== SIDEBAR / PARÂMETROS (Lógica de Carregamento V11.4) =====================
with st.sidebar:
    ...
    def get_padrao_from_sheets(sheet_id):
        ...
        # 1. Baixa o conteúdo e o carrega usando a função robusta (V11.4)
        content = baixar_xlsx_do_sheets(sheet_id)
        cat = _carregar_padrao_de_content(content) # Retorna DFs com coluna 'Preco' limpa e maiúscula
        
        df_cat = cat.catalogo_simples
        df_kits = cat.kits_reais
        
        # 2. Lógica de Carregamento de Preço de Estoque (Hyper-Defensive)
        try:
            df_precos_list = []
            for emp in ("ALIVVIA", "JCA"):
            ...
            if df_precos_list:
                ...
                df_precos_final = df_precos_all.drop_duplicates(subset=["SKU"], keep="last").rename(columns={'Preco': 'Preco_Estoque'})
                
                # PREPARAÇÃO DEFENSIVA
                
                # 1. Renomeia o Preco do Catálogo
                if 'Preco' in df_cat.columns:
                    df_cat = df_cat.rename(columns={'Preco': 'Preco_Cat'}).copy()
                    df_cat['Preco_Cat'] = df_cat['Preco_Cat'].fillna(0.0) # Garantido como float pelo loader
                else:
                    df_cat['Preco_Cat'] = 0.0
                    
                # 2. Mescla o preço do Estoque
                df_cat = df_cat.merge(df_precos_final, on="SKU", how="left")
                df_cat['Preco_Estoque'] = df_cat['Preco_Estoque'].fillna(0.0)
                
                # 3. SELEÇÃO FINAL: Catálogo > Estoque (Sem ambiguidades)
                ...
                # Limpeza final (como você sugeriu)
                df_cat = df_cat.drop(columns=['Preco_Cat', 'Preco_Estoque'], errors='ignore')

            # Se não houver Estoque, usa o preço do Catálogo (já limpo e renomeado para 'Preco')
            if 'Preco' not in df_cat.columns: 
                 df_cat['Preco'] = 0.0

        except Exception as e:
            # Garante que 'Preco' seja 0.0 em caso de erro no merge
    ...
    with colA:
        if st.button("Carregar padrão agora", use_container_width=True):
            try:
                # Limpa o cache para garantir que rode o código novo (V11.4)
    ...
    # CORREÇÃO DO WARNING: Widget criado apenas com KEY (valor inicializado em _ensure_state)
    st.text_input(
        "Link alternativo do Google Sheets (opcional)",
        key="alt_sheet_link",
        help="Se necessário, cole o link e use o botão abaixo."
        # 'value' removido para evitar o duplo binding.
    )
