# reposicao_OC_dev.py - VERSÃO FINAL DE PRODUÇÃO
import io
import os
import json
import re
...
# =======================================================
# --- FUNÇÕES DE PERSISTÊNCIA E ESTADO (MANTIDAS) ---
# [Funções de persistência de disco, _store_put, _ensure_state, etc. mantidas intactas]
# (Seu código original deve ter o corpo destas funções)
...
def _disk_put(emp: str, kind: str, name: str, blob: bytes): 
    ...
    with open(os.path.join(p, "file.bin"), "wb") as f: f.write(blob)
def _disk_get(emp: str, kind: str): return None
def _store_put(emp, kind, name, blob): st.session_state[emp][kind] = {"name": name, "bytes": blob}; _disk_put(emp, kind, name, blob)
def _store_get(emp: str, kind: str): return None
...
def load_any_table_from_bytes(file_name: str, blob: bytes) -> pd.DataFrame:
    ...
    try:
        # Tenta ler como excel, senão tenta outros formatos
...
# --- Mapeamento e Cálculo (Restaurado) ---
@dataclass
class Catalogo:
...
def mapear_tipo(df: pd.DataFrame) -> str:
    ...
    if tem_sku and (tem_v60 or tem_estoque_full): return "FULL"
    if tem_sku and tem_preco: return "FISICO"
    if tem_sku and not tem_preco: return "VENDAS"
    return "DESCONHECIDO"

def mapear_colunas(df: pd.DataFrame, tipo: str) -> pd.DataFrame:
    # Esta função estava no seu código original (apenas restaurada aqui para rodar)
    if tipo == "FULL":
...
def calcular(full_df, fisico_df, vendas_df, cat: "Catalogo", h=60, g=0.0, LT=0):
    ...
    # [Lógica de cálculo completa, restaurada do seu código original]
    
    # 1. Preparação
    kits = explodir_por_kits(cat.catalogo_simples.rename(columns={'component_sku': 'kit_sku'}), cat.kits_reais, 'kit_sku', 'qty').drop(columns=['Quantidade'])
    
    full = full_df.copy()
    shp = vendas_df.copy().rename(columns={"Quantidade": "Quantidade_60d"})

    # 2. Explodir Vendas (para achar a demanda de componentes)
    ...
    full["vendas_dia"] = full["Vendas_Qtd_60d"] / 60.0
    ...
    df_final = base[[
        "SKU", "fornecedor", "Estoque_Fisico", "Preco", "Compra_Sugerida", "Valor_Compra_R$",
        "ML_60d", "Shopee_60d", "TOTAL_60d", "Reserva_30d", "Necessidade"
...
# --- HTTP Google Sheets (Para KITS/CAT) ---
...
with tab_compra:
    ...
    if st.button(f"Gerar Compra — {empresa}", type="primary", key=f"btn_calc_{empresa}"):
        try:
            if not (dados["FULL"]["name"] and dados["VENDAS"]["name"] and dados["ESTOQUE"]["name"]):
        ...
        with st.expander("📋 Lista combinada ALIVVIA + JCA"):
            ...
            if not df_alivvia.empty and not df_jca.empty:
                ...
                cols_show2 = [
                    ...
                    "Vendas_ALIVVIA_60d", "Vendas_JCA_60d", "Vendas_Total_60d"
                ]
                dfV = dfV[[c for c in cols_show2 if c in dfV.columns]]
                dfV["Selecionar"] = False
                
                st.dataframe(dfV.drop(columns=["Vendas_ALIVVIA_60d", "Vendas_JCA_60d"]), use_container_width=True) # Exibe
