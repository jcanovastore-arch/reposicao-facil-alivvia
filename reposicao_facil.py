...
# - FULL por anúncio; compra por componente
# - Shopee explode antes; painel; prévia por SKU; filtro por fornecedor
# - Padrão (KITS/CAT) SEMPRE do Google Sheets (via export CSV)
# - Permite COLAR QUALQUER LINK do Sheets (edit/export) e extrai sheet_id + gid
# - Resultados ficam em session_state (sem recálculo nos filtros)
...
# ==========================
# Defaults (podem ser trocados pela UI colando os links)
# ==========================
DEFAULT_SHEET_ID = "1cTLARjq-B5g50dL6tcntg7lb_Iu0ta43"
DEFAULT_GID_KITS = "1589453187"  # KITS (você já confirmou)
DEFAULT_GID_CAT  = "0"           # CATALOGO (confira; se mudar, cole o link da aba e clique "Usar link p/ CAT")

# Estado inicial
if "SHEET_ID" not in st.session_state:
    st.session_state.SHEET_ID = DEFAULT_SHEET_ID
if "GID_KITS" not in st.session_state:
    st.session_state.GID_KITS = DEFAULT_GID_KITS
if "GID_CATALOGO" not in st.session_state:
    st.session_state.GID_CATALOGO = DEFAULT_GID_CAT
...
# Utils
# ==========================
def parse_google_sheets_link(url: str) -> tuple[str | None, str | None]:
    """
    Aceita links de edição (…/edit#gid=...) OU export (…/export?format=csv&gid=...).
    Retorna (sheet_id, gid_ou_None).
    """
    if not url:
        return None, None
...
def read_gs_csv(sheet_id: str, gid: str) -> pd.DataFrame:
    ...
    try:
        ...
        if status in (400, 401, 403):
            hint.append("• Verifique COMPARTILHAMENTO: 'Qualquer pessoa com o link' (Leitor).")
            hint.append("• Abra o link em aba anônima e veja se baixa o CSV.")
            hint.append("• Confirme o gid: abra a aba no Sheets e veja o número após '#gid='.")
        msg = f"Falha ao baixar CSV do Google Sheets (HTTP {status}): {url}\n" + ("\n".join(hint) if hint else "")
        raise RuntimeError(msg) from e
    except Exception as e:
        raise RuntimeError(f"Falha de rede ao baixar CSV: {url} | Erro: {e}") from e

    try:
        return pd.read_csv(io.BytesIO(r.content), dtype=str, keep_default_na=False)
    except Exception as e:
        raise RuntimeError(f"Não consegui ler o CSV do Google Sheets (conteúdo inválido): {url} | Erro: {e}") from e
...
@dataclass
class Catalogo:
    catalogo_simples: pd.DataFrame  # component_sku, fornecedor, (opcional) status_reposicao
    kits_reais: pd.DataFrame        # kit_sku, component_sku, qty (>=1)

# ==========================
# Padrão (Google Sheets)
# ==========================
@st.cache_data(ttl=300, show_spinner=False)
def load_gs_data() -> Catalogo:
    SHEET_ID = st.session_state.SHEET_ID
    GID_KITS = st.session_state.GID_KITS
    GID_CATALOGO = st.session_state.GID_CATALOGO
...
# ==========================
def load_any_table(uploaded_file) -> pd.DataFrame | None:
    ...
    # FULL com header na 3ª linha (alguns relatórios com 2 linhas de título)
...
# ==========================
# Detecção e mapeamento (robusto)
# ==========================
def mapear_tipo(df: pd.DataFrame) -> str:
    cols = [c.lower() for c in df.columns]

    def tem_col(pred):
        return any(pred(c) for c in cols)
    ...
    tem_estoque_full_like = tem_col(lambda c: ("estoque" in c and "full" in c) or c in {"estoque_full"})
    tem_estoque_generico  = tem_col(lambda c: c in {"estoque_atual","qtd","quantidade"} or "estoque" in c)
    tem_transito_like     = tem_col(lambda c: ("transito" in c) or c in {"em_transito","em transito","em_transito_full","em_transito_do_anuncio"})
...
def calcular(full_df, fisico_df, vendas_df, cat: Catalogo, h=60, g=0.0, LT=0):
    ...
    # Shopee/MT → já 60d (arquivo de vendas por SKU)
    shp = vendas_df.copy()
...
def exportar_xlsx(df_final: pd.DataFrame, h: int, params: dict, pendencias: list | None = None) -> bytes:
    ...
    for c in int_cols:
        ...
        if len(bad) > 0:
            linha = int(bad[0]) + 2  # +1 header +1 base 1
...
st.caption("FULL por anúncio; compra por componente; Shopee explode antes; painel; prévia por SKU; filtro por fornecedor. Resultados em memória (sem recálculo).")
...
with st.sidebar:
    ...
    # Cole QUALQUER link do Sheets (edit/export)
    link_colado = st.text_input(
        "Cole o link da planilha/aba (edit ou export)",
        value=f"https://docs.google.com/spreadsheets/d/{st.session_state.SHEET_ID}/edit"
    )

    colA, colB = st.columns(2)
    with colA:
        if st.button("Usar link p/ KITS"):
            sid, gid = parse_google_sheets_link(link_colado.strip())
            if sid: st.session_state.SHEET_ID = sid
            if gid: st.session_state.GID_KITS = gid
            st.success(f"KITS → sheet_id={st.session_state.SHEET_ID} gid={st.session_state.GID_KITS}")
    with colB:
        if st.button("Usar link p/ CATALOGO"):
            sid, gid = parse_google_sheets_link(link_colado.strip())
            if sid: st.session_state.SHEET_ID = sid
            if gid: st.session_state.GID_CATALOGO = gid
            st.success(f"CATALOGO → sheet_id={st.session_state.SHEET_ID} gid={st.session_state.GID_CATALOGO}")

    st.caption("Ajuste manual (opcional):")
    ...
    abrir_url = f"https://docs.google.com/spreadsheets/d/{st.session_state.SHEET_ID}/edit"
...
# Empresa perto dos uploads
st.subheader("Empresa")
empresa = st.radio("Empresa ativa", ["ALIVVIA", "JCA"], horizontal=True)

# Carrega padrão (Sheets)
try:
    cat = load_gs_data()
    KITS = cat.kits_reais
    CATALOGO = cat.catalogo_simples.rename(columns={"component_sku":"sku"})
except Exception as e:
...
if st.button(f"Gerar Compra — {empresa}", type="primary"):
    try:
        ...
        for df_up in [full_raw, fisico_raw, vendas_raw]:
            t = mapear_tipo(df_up)
            if t == "DESCONHECIDO":
                st.error("Um dos arquivos enviados não foi reconhecido (FULL/FISICO/VENDAS). Reexporte com colunas corretas.")
                st.stop()
