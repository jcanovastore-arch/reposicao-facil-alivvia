...
# Duas empresas (separação real de estado, uploads e resultados)
EMPRESAS = ["ALIVVIA", "JCA"]

# Arquivos locais
PADRAO_LOCAL_NOME = "Padrao_produtos.xlsx"  # salvo ao baixar do Google
APP_STATE_FILE = "estado_app.json"          # estado persistente (JSON)

# Google Sheets - ID padrão da sua planilha KITS/CAT
...
def get_state() -> dict:
    if "STATE" not in st.session_state:
        ...
        for emp in EMPRESAS:
            ...
            st.session_state.STATE[emp].setdefault("compras_manuais", [])  # lista de dicts
...
def calcular(full_df, fisico_df, vendas_df, cat: Catalogo, h=60, g=0.0, LT=0):
    ...
    # Vendas no horizonte h (exibição)
    ...
    # Painel de estoques (como no seu app)
...
if STATE.get("auto_baixar_padrao") and not st.session_state.get("baixou_padrao_auto"):
    ...
    baixar_padrao_do_drive(url_export, PADRAO_LOCAL_NOME)
