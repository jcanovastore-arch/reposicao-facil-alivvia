...
# v3.2.4 - 2025-10-17
# - Limpar arquivo individual (FULL/VENDAS/ESTOQUE)
# - Fix largura do Excel quando DF vazio (evita ValueError)
# - Download "Compra_2Contas.xlsx" (lista combinada ALIVVIA + JCA) com função robusta
# - Mantém UI enxuta, filtros pós-cálculo, consolidação por SKU, persistência
...
def _store_delete(emp: str, kind: str):
    store = _file_store()
    store[emp][kind] = None
    st.session_state[emp][kind] = {"name": None, "bytes": None}
...
def baixar_xlsx_por_link_google(url: str) -> bytes:
    ...
    r = s.get(gs_export_xlsx_url(sid), timeout=30); r.raise_for_status()
    return r.content

def baixar_xlsx_do_sheets(sheet_id: str) -> bytes:
    s = _requests_session()
    r = s.get(gs_export_xlsx_url(sheet_id), timeout=30); r.raise_for_status()
    return r.content
...
def _carregar_padrao_de_content(content: bytes) -> "Catalogo":
    ...
    def load_sheet(opts):
        ...
        raise RuntimeError(f"Aba não encontrada. Abas: {xls.sheet_names}")
...
def mapear_colunas(df: pd.DataFrame, tipo: str) -> pd.DataFrame:
    ...
    if tipo=="FISICO":
        ...
        df["Estoque_Fisico"]=pd.to_numeric(df[c_q[0]], errors="coerce").fillna(0).clip(lower=0).astype(int)
        ...
        df["Preco"]=pd.to_numeric(df[c_p[0]], errors="coerce").fillna(0.0)
    ...
    if tipo=="VENDAS":
        ...
        df["Quantidade"]=pd.to_numeric(df[qcol], errors="coerce").fillna(0).astype(int)
...
def calcular(full_df, fisico_df, vendas_df, cat: "Catalogo", h=60, g=0.0, LT=0):
    ...
    demanda[["ML_60d","Shopee_60d"]] = (
        demanda[["ML_60d","Shopee_60d"]].apply(pd.to_numeric, errors="coerce").fillna(0).clip(lower=0).astype(int)
    )
...
def _safe_width(df: pd.DataFrame, col: str, minimo: int = 12) -> int:
    if df.empty or col not in df.columns:
        return minimo
    ml = df[col].astype(str).map(len).max()
    ml = 0 if pd.isna(ml) else int(ml)
    return max(minimo, ml + 2)
...
def exportar_xlsx(df_final: pd.DataFrame, h: int, params: dict) -> bytes:
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as w:
        ...
        for i,col in enumerate(lista.columns):
            ws.set_column(i,i, min(_safe_width(lista,col),40))
        ws.freeze_panes(1,0)
    ...
    with _pd.ExcelWriter(bio, engine="xlsxwriter") as w:
        ...
        for i, col in enumerate(df.columns):
            if df.empty:
                ...
                ml = df[col].astype(str).map(len).max()
                ml = 0 if _pd.isna(ml) else int(ml)
                width = max(12, ml + 2)
...
def bloco_empresa(emp: str):
    ...
    with c2:
        st.write("")
        if st.button("Limpar apenas FULL", use_container_width=True, key=f"clr_full_{emp}"):
            _store_delete(emp,"FULL"); st.info("FULL apagado.")
    ...
    with c2:
        st.write("")
        if st.button("Limpar apenas Vendas", use_container_width=True, key=f"clr_v_{emp}"):
            _store_delete(emp,"VENDAS"); st.info("Vendas apagado.")
    ...
    with c2:
        st.write("")
        if st.button("Limpar apenas Estoque", use_container_width=True, key=f"clr_e_{emp}"):
            _store_delete(emp,"ESTOQUE"); st.info("Estoque apagado.")

    # Confirmar/limpar geral
    colx, coly = st.columns(2)
    with colx:
        if st.button(f"Salvar {emp}", use_container_width=True, key=f"save_{emp}"):
            for kind in ["FULL","VENDAS","ESTOQUE"]:
                it = st.session_state[emp][kind]
                if it["name"] and it["bytes"]: _disk_put(emp,kind,it["name"],it["bytes"])
    ...
    with coly:
        if st.button(f"Limpar {emp}", use_container_width=True, key=f"clr_{emp}"):
            ...
            _store_clear(emp); st.info(f"{emp} limpo.")
    st.divider()
    ...
    if st.session_state.catalogo_df is None or st.session_state.kits_df is None:
        ...
        if st.button(f"Gerar Compra — {empresa}", type="primary", key=f"btn_calc_{empresa}"):
            try:
                ...
                t_full, t_v, t_f = mapear_tipo(full_raw), mapear_tipo(vendas_raw), mapear_tipo(fisico_raw)
        ...
        if empresa in st.session_state["resultado_compra"]:
            ...
            df_final, painel = pkg["df"], pkg["painel"]
            ...
            cols_show = ["fornecedor","SKU","Vendas_h_Shopee","Vendas_h_ML","Estoque_Fisico","Preco","Compra_Sugerida","Valor_Compra_R$"]
            df_view_sub = df_view[[c for c in cols_show if c in df_view.columns]].copy()

            st.caption(f"Linhas após filtros: {len(df_view_sub)}")
            st.dataframe(
                df_view_sub, use_container_width=True, height=500, hide_index=True,
                column_config={
            ...
            # -------------- COMBINADO (ALIVVIA + JCA) --------------
            st.markdown("---")
            with st.expander("📄 Baixar lista combinada (ALIVVIA + JCA)", expanded=False):
                tem_A = "ALIVVIA" in st.session_state["resultado_compra"]
                tem_J = "JCA" in st.session_state["resultado_compra"]
                if not (tem_A and tem_J):
                    st.info("Gere a compra para ALIVVIA e JCA para habilitar.")
                else:
                    dfA = st.session_state["resultado_compra"]["ALIVVIA"]["df"][["fornecedor","SKU","Compra_Sugerida","Valor_Compra_R$"]].rename(
                        columns={"Compra_Sugerida":"Compra_ALIVVIA","Valor_Compra_R$":"Valor_ALIVVIA"}
                    )
                    dfJ = st.session_state["resultado_compra"]["JCA"]["df"][["fornecedor","SKU","Compra_Sugerida","Valor_Compra_R$"]].rename(
                        columns={"Compra_Sugerida":"Compra_JCA","Valor_Compra_R$":"Valor_JCA"}
                    )
                    dfV = pd.merge(dfA, dfJ, on=["fornecedor","SKU"], how="outer")
                    for c in ["Compra_ALIVVIA","Valor_ALIVVIA","Compra_JCA","Valor_JCA"]:
                        dfV[c] = pd.to_numeric(dfV[c], errors="coerce").fillna(0)
                    dfV["Compra_Total"] = (dfV["Compra_ALIVVIA"] + dfV["Compra_JCA"]).astype(int)
                    dfV["Valor_Total"]  = (dfV["Valor_ALIVVIA"] + dfV["Valor_JCA"]).round(2)
                    dfV = dfV.sort_values(["fornecedor","SKU"]).reset_index(drop=True)
                ...
                if not (tem_A and tem_J):
                    ...
                    if not sku_all:
                        ...
                        def pick(df, sku):
                            ...
                            if row.empty: return {"SKU":sku,"Compra_Sugerida":0,"Valor_Compra_R$":0.0}
                            r=row.iloc[0].to_dict()
                            r["Compra_Sugerida"]=int(r.get("Compra_Sugerida",0))
                            r["Valor_Compra_R$"]=float(r.get("Valor_Compra_R$",0))
                            return r
                        rA, rJ = pick(dfA, sku_sel), pick(dfJ, sku_sel)
                        res = pd.DataFrame([
                        ...
                        st.success(f"{sku_sel} → ALIVVIA: {rA['Compra_Sugerida']} | JCA: {rJ['Compra_Sugerida']}")
    ...
    if st.session_state.catalogo_df is None or st.session_state.kits_df is None:
        ...
        if st.button("Calcular alocação proporcional", type="primary"):
            try:
                missing=[]
                for emp in ["ALIVVIA","JCA"]:
                    if not (st.session_state[emp]["FULL"]["name"] and st.session_state[emp]["FULL"]["bytes"]): missing.append(f"{emp} FULL")
                    if not (st.session_state[emp]["VENDAS"]["name"] and st.session_state[emp]["VENDAS"]["bytes"]): missing.append(f"{emp} Shopee/MT")
                if missing: raise RuntimeError("Faltam arquivos: " + ", ".join(missing))
                ...
                def read_pair(emp: str) -> Tuple[pd.DataFrame,pd.DataFrame]:
                    ...
                    tfa, tsa = mapear_tipo(fa), mapear_tipo(sa)
                ...
                cat = Catalogo(CATALOGO.rename(columns={"sku":"component_sku"}), st.session_state.kits_df)
                kits=construir_kits_efetivo(cat)
                def vendas_comp(full_df, shp_df):
                    ...
                    out=pd.merge(a,b,on="SKU",how="outer").fillna(0); out["Demanda_60d"]=out["ML_60d"].astype(int)+out["Shopee_60d"].astype(int); return out[["SKU","Demanda_60d"]]
                demA, demJ = vendas_comp(full_A, shp_A), vendas_comp(full_J, shp_J)
                sku_norm=norm_sku(sku_escolhido); dA=int(demA.loc[demA["SKU"]==sku_norm,"Demanda_60d"].sum()); dJ=int(demJ.loc[demJ["SKU"]==sku_norm,"Demanda_60d"].sum())
                total=dA+dJ
                if total==0: propA=propJ=0.5
                else: propA,propJ=dA/total, dJ/total
                alocA=int(round(qtd_lote*propA)); alocJ=int(qtd_lote-alocA)
                res=pd.DataFrame([
                ...
                st.success(f"Total alocado: {qtd_lote} (ALIVVIA {alocA} | JCA {alocJ})")
                st.download_button("Baixar alocação (.csv)", data=res.to_csv(index=False).encode("utf-8"), file_name=f"Alocacao_{sku_norm}_{qtd_lote}.csv", mime="text/csv")
            except Exception as e:
                st.error(str(e))
