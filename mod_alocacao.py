# mod_alocacao.py - TAB 3 - V11.0 (Sincronização Total)
# - FIX: Corrige o KeyError: "'Preco' not in index" (V10.12)
# - A função 'calcular_proporcoes_venda' lê os arquivos de ESTOQUE para obter o "Preco"
...
def calcular_proporcoes_venda(_state):
    """
    Calcula a demanda de 60 dias por componente para ALIVVIA e JCA,
    e retorna o DataFrame mesclado com a proporção e o preço (dos estoques).
    """
    missing = []
    files_map = {}
    
    # LÊ FULL, VENDAS e ESTOQUE para ambas as empresas
    ...
    def read_vendas_pair(emp: str) -> tuple[pd.DataFrame, pd.DataFrame]:
        fa_name, fa_bytes = files_map[f"{emp}_FULL"]; sa_name, sa_bytes = files_map[f"{emp}_VENDAS"]
        fa = load_any_table_from_bytes(fa_name, fa_bytes); sa = load_any_table_from_bytes(sa_name, sa_bytes)
    ...
    demandas_finais["Prop_A"] = demandas_finais.apply(
        lambda row: 0.5 if row["Demanda_Total"] == 0 else row["Demanda_A"] / row["Demanda_Total"], axis=1
    )
    ...
    demandas_finais = demandas_finais.merge(
        df_catalogo[['sku', 'fornecedor']].rename(columns={'sku':'SKU'}), on="SKU", how="left"
    )
    demandas_finais = demandas_finais.merge(
        df_precos, on="SKU", how="left"
    )
...
def render_tab3(state):
    ...
    try:
        ...
        if st.button("Calcular Alocação Proporcional", type="primary"):
            ...
            # Limpa o cache antes de rodar, para garantir que os arquivos de estoque mais recentes sejam usados.
            calcular_proporcoes_venda.clear() 
            ...
            propA = item["Prop_A"]; propJ = item["Prop_J"]
            ...
            state.alocacao_manual_calculada = [
                {
                    ...
                    "Valor_Compra_R$": round(alocA * preco_item, 2), # Adiciona Valor_Compra_R$
                    "Empresa": "ALIVVIA"
                },
                {
                    "SKU": sku_escolhido,
                    ...
                    "Valor_Compra_R$": round(alocJ * preco_item, 2), # Adiciona Valor_Compra_R$
                    "Empresa": "JCA"
                }
            ]
            ...
            res = pd.DataFrame([
                {"Empresa": "ALIVVIA", "Demanda_60d": item["Demanda_A"], "Proporção": propA, "Alocação_Sugerida": alocA, "Preço": preco_item},
                {"Empresa": "JCA", "Demanda_60d": item["Demanda_J"], "Proporção": propJ, "Alocação_Sugerida": alocJ, "Preço": preco_item},
            ])
        ...
        if "alocacao_manual_calculada" in state:
            ...
            if st.button("➡️ Enviar esta Alocação para Cesta de OC (Tab 4)", type="secondary"):
                try:
                    ...
                    # Garantindo que o DataFrame de envio tenha as colunas necessárias (V10.15)
                    colunas_oc = ["SKU", "fornecedor", "Preco", "Compra_Sugerida", "Valor_Compra_R$"]
                    
                    df_alivvia = pd.DataFrame([i for i in itens if i["Empresa"] == "ALIVVIA"])[colunas_oc]
                    df_jca = pd.DataFrame([i for i in itens if i["Empresa"] == "JCA"])[colunas_oc]
                    ...
                    del state.alocacao_manual_calculada
