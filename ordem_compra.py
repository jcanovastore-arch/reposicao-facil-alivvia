...
# --- GESTÃO DA CESTA (RAM) ---
def _init_cesta():
    # CHAVE CORRETA USADA PELO V10.5
...
def display_oc_interface(df_reposicao_final):
    ...
    if not cesta_itens:
        st.info("🛒 A Cesta está vazia. Selecione itens na 'Compra Automática' (Tab 2) ou 'Alocação' (Tab 3).")
        return
...
def render_tab4(state):
    ...
    # A 'state' (st.session_state) é passada pelo V10.3, mas esta função usa
    # o st.session_state.oc_cesta_itens (que é a chave correta pós V10.5).
